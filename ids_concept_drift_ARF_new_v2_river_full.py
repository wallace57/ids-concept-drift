...
# Always available in river.drift
from river.drift import ADWIN, PageHinkley

# DDM + HDDM_* thường nằm trong river.drift.binary ở các bản mới
try:
    from river.drift.binary import DDM, HDDM_A, HDDM_W
except Exception:
    # fallback cho vài bản cũ hơn
    from river.drift import DDM, HDDM_A, HDDM_W  # type: ignore

# KSWIN đôi khi export trực tiếp, đôi khi không -> import robust
try:
    from river.drift import KSWIN
except Exception:
    # fallback path (trường hợp hiếm)
    from river.drift.kswin import KSWIN  # type: ignore
...
def build_arf_variants():
    common = dict(n_models=10, seed=RANDOM_SEED)

    return {
        # 1) None
        "ARF_None": forest.ARFClassifier(**common),

        # 2) PageHinkley
        "ARF_PageHinkley": forest.ARFClassifier(
            **common, drift_detector=PageHinkley(), warning_detector=PageHinkley()
        ),

        # 3) ADWIN
        "ARF_ADWIN": forest.ARFClassifier(
            **common, drift_detector=ADWIN(), warning_detector=ADWIN()
        ),

        # 4) KSWIN
        "ARF_KSWIN": forest.ARFClassifier(
            **common, drift_detector=KSWIN(), warning_detector=KSWIN()
        ),

        # 5) HDDM_A
        "ARF_HDDM_A": forest.ARFClassifier(
            **common, drift_detector=HDDM_A(), warning_detector=HDDM_A()
        ),

        # 6) HDDM_W
        "ARF_HDDM_W": forest.ARFClassifier(
            **common, drift_detector=HDDM_W(), warning_detector=HDDM_W()
        ),

        # 7) DDM
        "ARF_DDM": forest.ARFClassifier(
            **common, drift_detector=DDM(), warning_detector=DDM()
        ),
    }
...
def plot_all_models(periods, all_results):
    """
    Vẽ Accuracy và F1-score cho:
    Static + tất cả ARF variants
    """
    T = len(periods)
    x = np.arange(1, T + 1)
    ...
    plt.xlabel("Period")
    plt.ylabel("Accuracy")
    plt.title("Accuracy over Periods (Static vs ARF Variants)")
    plt.ylim(0.0, 1.0)
    plt.grid(True, alpha=0.3)
    ...
    plt.xlabel("Period")
    plt.ylabel("F1-score")
    plt.title("F1-score over Periods (Static vs ARF Variants)")
    plt.ylim(0.0, 1.0)
    plt.grid(True, alpha=0.3)
    plt.legend(loc="lower left", fontsize=9, ncol=2)
    plt.tight_layout()
...
def main():
    ...
    print("\n[6] Plotting ALL models (Static + 7 ARF variants)...")
    plot_all_models(periods, all_results)
